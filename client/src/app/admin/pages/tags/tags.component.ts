...
export class TagsComponent implements OnInit, OnDestroy {
  ...
  public addTag(value: string) {
    this.tagsService.createTag(value).pipe(takeUntil(this.destroy$)).subscribe({
      next: (value) => {
  ...
  public removeTag(tag: TagInterface) {
    console.log(tag);
    this.dialogService.openDialog(DialogTypeEnum.ConfirmDeleteTag, {
      title: 'ПОВІДОМЛЕННЯ',
      text: '',
      user: undefined,
      tag: tag,
  ...
  private confirmRemoveTag(id: number) {
    this.tagsService.removeTag(id).pipe(takeUntil(this.destroy$)).subscribe({
      next: () => {
